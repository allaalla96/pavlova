...
global $USER;
###
\Bitrix\Main\Loader::includeModule('mycompany.statistic');
use MYCOMPANY\STATISTIC;
...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"] === "N"
 ...
 'VALUE' => '01.' . $i . '.' . date('Y')
    ...
    ###
    ###Получение списка показателей статистики продаж для сотрудника###
    $arResult['ITEMS'] =
        STATISTIC\StatManager::GetStatEmployee($arParams["USER_ID"]);


    foreach($arResult['ITEMS'] as &$arStat) {
        $arValue = STATISTIC\StatManager::GetStatEmployeeValue($arStat["ID"], $arParams["USER_ID"], $_REQUEST['UF_PERIOD']);
        $arStat["STAT_ID"] = $arValue["ID"];
        $arStat["STAT_VALUE"] = $arValue["UF_VALUE"];
    }
